...
N_ELECTRODES = 105
N_ET_VARIABLES = 5 # just to make sure the dummy et for padding is 5 variables long
...
# The definition below is replaced with the new definition written after it using tensorflow Tokenizer and pad_sequences, 
# to make it compatible with the latest tensorflow version in 2023.
'''
...
# Below is the New definition code by Yingjia Wan. Changes include:
    # Importing Tokenizer from tensorflow.keras.preprocessing.text instead of tflearn.data_utils.
    # Importing pad_sequences from tensorflow.keras.preprocessing.sequence to pad the sequences.
    # Creating a Tokenizer object (still called 'vocab_processor') instead of a Vocabulary_processor and fitting it on the training data using fit_on_texts.
    # Converting the text to sequences using texts_to_sequences.
...
def get_processed_dataset(dataset_path, binary, verbose, labels_from):
    ...
    :return:
        ...
        x_text:             (list)  List of all sentences
        y:                  (list)  List of lists, each sublist is the OHE vector of the response variable
        tokenizer:          (tfobj) Converts word idx to word and vice versa
    """
    ...
    # Build tensorflow tokenizer (but we still call it vocab_processor to avoid issues with calling it in other places)
    vocab_processor = Tokenizer()
    vocab_processor.fit_on_texts(x_text)
    x = vocab_processor.texts_to_sequences(x_text)
    x = pad_sequences(x, padding='post')

    if(verbose):
        print("Vocabulary Size: {:d}".format(len(vocab_processor.word_index)))
        print("Max. Sentence Length: {:d}".format(x.shape[1]))
...
class new_data_box:
    ...
    # This function pads the missing eyetracking data with zeros.
        #TODO: add another element with "fixations" isolated from eeg and
        n_var_et = N_ET_VARIABLES
        dummy_et = np.zeros(n_var_et)
